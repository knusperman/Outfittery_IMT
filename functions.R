...
ggplot(cluster2wss, aes(x=clusters, y = ssw))+geom_line()+geom_point()+scale_x_continuous(breaks=1:15)+scale_y_continuous(breaks= seq(1000,7000,1000))+xlab("number of clusters")+ylab("sum of squares within clusters")

}
clusterplot.percentage <- function(data){
	#input is clusterdata from function clustering
	data <- data[2:3]
wss <- kmeans(clusterdatar,1 )$totss
for (i in 2:15) wss[i] <- (kmeans(clusterdatar, i)$totss - kmeans(clusterdatar,i)$tot.withinss)/kmeans(clusterdatar, i)$totss
